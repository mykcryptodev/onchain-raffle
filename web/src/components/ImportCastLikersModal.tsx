...
interface ImportCastLikersModalProps {
  isOpen: boolean;
  onClose: () => void;
  onImport: (addresses: string[]) => void;
}

export const ImportCastLikersModal: FC<ImportCastLikersModalProps> = ({ isOpen, onClose, onImport }) => {
  const [castHash, setCastHash] = useState('');
  const [likers, setLikers] = useState<NeynarUser[]>([]);
  const [selectedUsers, setSelectedUsers] = useState<SelectedUser[]>([]);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState('');
  const [activeTab, setActiveTab] = useState<'hash' | 'selected'>('hash');

  const fetchCastLikers = async () => {
    if (!castHash.trim()) {
    ...
    try {
      const response = await fetch('/api/neynar/cast-likes', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        throw new Error(data.error || 'Failed to fetch cast likers');
      }
      ...
      setError(error instanceof Error ? error.message : 'Failed to fetch cast likers');
      setLikers([]);
      setSelectedUsers([]);
    } finally {
      setIsLoading(false);
      ...
      <div className="fixed inset-0 flex items-end sm:items-center justify-center z-50 p-0 sm:p-4">
        ...
        >
          ...
          <div className="p-4 sm:p-6 border-b border-zinc-700 flex-shrink-0">
            <h2 className="text-xl sm:text-2xl font-bold mb-2">Import Cast Likers</h2>
            <p className="text-sm text-zinc-400">
              Enter a Farcaster cast hash to import Ethereum addresses of users who liked the cast.
            </p>
          </div>
          ...
          <div className="flex-1 min-h-0 overflow-hidden">
            ...
            <div className={`md:hidden h-full ${activeTab !== 'hash' ? 'hidden' : ''}`}>
              <div className="p-4">
                {likers.length > 0 && (
                  <div className="mb-4 p-4 bg-zinc-800 rounded-lg">
                    ...
                    <div className="text-sm text-zinc-400 space-y-1">
                      <div>Total Likes: {likers.length}</div>
                ...
                {selectedUsers.length === 0 && likers.length === 0 && !isLoading && (
                  <p className="text-zinc-400 text-center py-8">Enter a cast hash above to get started</p>
                )}
                
                {selectedUsers.length === 0 && likers.length > 0 && (
            ...
            <div className="hidden md:flex h-full">
              ...
              <div className="w-1/2 p-4 border-r border-zinc-700 overflow-y-auto">
                ...
                {selectedUsers.length === 0 && likers.length > 0 && (
                  <p className="text-zinc-400 text-sm">No users with Ethereum addresses found in the likes</p>
                )}
              </div>
